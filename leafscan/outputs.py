...
def save_png(path, img01, bits=8):
    ...
    OpenCV reliably writes 16-bit multi-channel PNGs (Pillow/imageio do not).
    """
    import cv2
    path = Path(path)
...
def write_outputs(out_dir, normal, albedo_rgb, valid, height=None,
    ...
    if albedo_linear:
        written.append(save_png(out / "albedo.png", np.clip(albedo_rgb, 0, 1), 16))
    if albedo_srgb:
        written.append(save_png(out / "albedo_srgb.png",
                                linear_to_srgb(np.clip(albedo_rgb, 0, 1)), 8))
